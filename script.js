...
function addBotProfileAvatar() {
    ...
    const selectedAvatar = localStorage.getItem('selectedAvatar'); // 动态获取机器人头像
    const avatarSrc = `${selectedAvatar}.png`; // 头像路径
...
function addMessage(sender, message, isAudio = false, audioDuration = null, audioSrc = null) {
    ...
    if (sender === 'user') {
        ...
        const selectedAvatar = localStorage.getItem('selectedAvatar'); // 获取机器人头像
